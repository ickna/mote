...
function getParticleColor(pt, p, rgb1, rgb2, rgb3) {
  ...
  // Palette cycling: shift t through the palette as a continuous gradient
  if (p.colorAnimEnabled && p.colorMode !== 'gradient-angular') {
    var pal = PALETTES[p.palette] || PALETTES.custom;
    var cols = pal.colors;
    var n = cols.length;
    if (n >= 2) {
      var phase = (p.colorAnimPhase || 0) * (p.colorAnimSpeed || 0.5) * 0.2;
      // Continuous position through the palette: t shifted by phase
      var pos = ((t + phase) % 1) * n;
      var idx = Math.floor(pos) % n;
      var nextIdx = (idx + 1) % n;
      var frac = pos - Math.floor(pos);
      return lerpRgb(hexToRgb(cols[idx]), hexToRgb(cols[nextIdx]), frac);
    }
  }
  // Static 3-color lerp (no animation): t=0 -> color1, t=0.5 -> color3, t=1 -> color2
